...
void mobil2(double **coord,const struct LennardJones* const lj,const double* const charge, size_t n_atoms,const struct Molecule *const mol, unsigned int iic, double *mob,double *cs,double *sdevpc,double (*RanNum)(const gsl_rng *r),const gsl_rng *r,FILE *fp,int print_im2, int *ifailc){
    ...
    */    
    ...
    unsigned int i;
    int ihold=0,irn,ir,ibst,ig,ic,im,icc;
    ...
    double q1st[INP],q2st[INP],om11st[ITN],om12st[ITN],om13st[ITN],om22st[ITN],temp1,temp2,rnb,invImp,mom11st=0.,mom12st=0.,mom13st=0.,mom22st=0.,sdom11st;
    ...
    if (!print_im2 && my_rank==0) fprintf(fp," along x axis emax =% -.4EeV rmax =% -.4EA r00 =% -.4EA\n",emaxx/XE,rmaxx*1.0e10,r00x*1.0e10);
    ...
    if (!print_im2 && my_rank==0) fprintf(fp," along y axis emax =% -.4EeV rmax =% -.4EA r00 =% -.4EA\n",emaxy/XE,rmaxy*1.0e10,r00y*1.0e10);
    ...
    if (!print_im2 && my_rank==0) fprintf(fp," along z axis emax =% -.4EeV rmax =% -.4EA r00 =% -.4EA\n\n",emaxz/XE,rmaxz*1.0e10,r00z*1.0e10);
    ...
    for(i=0;i<INP;i++){
        ...
        gst2[i]=pgst[i]*pgst[i]; // TODO: This is calculated twice, see hold1 in block above and vList are the same
    ...
    memset(q1st,0,INP*sizeof(double));
    memset(q2st,0,INP*sizeof(double));
    memset(om11st,0,ITN*sizeof(double));
    memset(om12st,0,ITN*sizeof(double));
    memset(om13st,0,ITN*sizeof(double));
    memset(om22st,0,ITN*sizeof(double));
    ...
    for(ic=0;ic<ITN;ic++){
        ...
        for(ig=0;ig<INP;ig++){
            ...
            for(im=0;im<IMP;im++){
                ...
                #if PRINT_IGS
                    char buffer[MAX_LINE];
                    sprintf(buffer,"thread_%d",my_rank);
                    ftemp=fopen(buffer,"w");
    ...
    for(icc=0;icc<ITN;icc++){
        tmp=1.0/(mol->mobility/(sqrt(mol->temperature)*om11st[icc]*M_PI*RO2));
        hold1+=om11st[icc];
        hold2+=tmp;
        if(!print_im2 && my_rank==0) fprintf(fp," %3d    % -.4E    % -.4E    % -.4E    % -.4E\n",icc+1,om11st[icc]*M_PI*RO2*1.e20,hold1*M_PI*RO2*1e20/((double)(icc+1)),tmp,hold2/((double)icc+1));        
    }
    
    if(!print_im2 && my_rank==0) fprintf(fp,"\n\n average values for q1st\n\n     gst2        wgst        q1st\n");
    if(!print_im2 && my_rank==0) for(ig=0;ig<INP;ig++) fprintf(fp," % -.4E % -.4E % -.4E\n",pgst[ig]*pgst[ig],wgst[ig],q1st[ig]/(double) INP);    
    for(ic=0;ic<ITN;ic++) mom11st+=om11st[ic];
    for(ic=0;ic<ITN;ic++) mom12st+=om12st[ic];
    for(ic=0;ic<ITN;ic++) mom13st+=om13st[ic];
    for(ic=0;ic<ITN;ic++) mom22st+=om22st[ic];

    double g_mom11st,g_mom12st,g_mom13st,g_mom22st;
    MPI_Reduce(&mom11st, &g_mom11st, 1, MPI_DOUBLE, MPI_SUM, 0,MPI_COMM_WORLD);
    MPI_Reduce(&mom11st, &g_mom11st, 1, MPI_DOUBLE, MPI_SUM, 0,MPI_COMM_WORLD);
    MPI_Reduce(&mom12st, &g_mom12st, 1, MPI_DOUBLE, MPI_SUM, 0,MPI_COMM_WORLD);
    MPI_Reduce(&mom13st, &g_mom13st, 1, MPI_DOUBLE, MPI_SUM, 0,MPI_COMM_WORLD);
    MPI_Reduce(&mom22st, &g_mom22st, 1, MPI_DOUBLE, MPI_SUM, 0,MPI_COMM_WORLD);

    if (my_rank==0){
        g_mom11st/=((double) (ITN * nprocs));
        g_mom12st/=((double) (ITN * nprocs));
        g_mom13st/=((double) (ITN * nprocs));
        g_mom22st/=((double) (ITN * nprocs));
    }
    MPI_Bcast(&g_mom11st,1,MPI_DOUBLE,0,MPI_COMM_WORLD);
    
    //standard deviation
    double g_sdom11st=0.0;
    sdom11st=0.0;
    for(ic=0;ic<ITN;ic++) sdom11st+=(om11st[ic]-g_mom11st)*(om11st[ic]-g_mom11st);
    MPI_Reduce(&sdom11st, &g_sdom11st, 1, MPI_DOUBLE, MPI_SUM, 0,MPI_COMM_WORLD);
    if (my_rank==0){
        g_sdom11st=sqrt(g_sdom11st/(double) (ITN * nprocs));
        sterr=g_sdom11st/sqrt((double)(ITN * nprocs));
    

        if(!print_im2) fprintf(fp,"\n\n mean OMEGA*(1,1) =% -.4E\n standard deviation =% -.4E\n standard error of mean =% -.4E\n",g_mom11st,g_sdom11st,sterr);
        *cs=g_mom11st*M_PI*RO2;
        *sdevpc=100.0*g_sdom11st/g_mom11st;
    
        //Use omegas to obtain higher order correction factor to mobility

        ayst=g_mom22st/g_mom11st;
        best=((5.0*g_mom12st)-(4.0*g_mom13st))/g_mom11st;
        cest=g_mom12st/g_mom11st;
...
void mobil4(double **coord,struct HardSphere* hs,const struct Molecule *const mol,double (*RanNum)(const gsl_rng *r),const gsl_rng *r,double *ehscs,double *ehsmob,double *pacs,double *pamob,unsigned int * imm,size_t n_atoms,FILE *fp,int print_im4){
    ...
    */    
    ...
    double crof[INOR],crb=0.,theta=0.,phi=0.,gamma=0.,ymin=0.,ymax=0.,zmin=0.,zmax=0.,ydi,zdi,pls,yr,zr;
    ...
    MPI_Comm_size(MPI_COMM_WORLD, &nprocs);
    newVector=AllocDoubleMat(n_atoms,3);
    memset(crof,0,INOR*sizeof(double));
    ...
    *imm=1;
    ...
    for(i=0;i<INUM;i++){
        ...
        #if (PRINT_IU2)
        fprintf(fp,"         initial coordinates                        new coordinates\n\n");
        for(n=0;n<n_atoms;n++) fprintf(fp," % -.4E % -.4E % -.4E      % -.4E % -.4E % -.4E\n",coord[n][0],coord[n][1],coord[n][2],newVector[n][0],newVector[n][1],newVector[n][2]);
        #endif

        // Build  a  box around the molecule (the particle comes from the x direction)
        ymin=0.;ymax=0.;zmin=0.;zmax=0.;
        for(n=0;n<n_atoms;n++){
            if (newVector[n][1]-hs->rhs[n]<ymin) ymin=newVector[n][1]-hs->rhs[n];
            if (newVector[n][1]+hs->rhs[n]>ymax) ymax=newVector[n][1]+hs->rhs[n];
            if (newVector[n][2]-hs->rhs[n]<zmin) zmin=newVector[n][2]-hs->rhs[n];
            if (newVector[n][2]+hs->rhs[n]>zmax) zmax=newVector[n][2]+hs->rhs[n];
        }
        ...
        zr=zmin+zdi*(*RanNum)();
        #else
        yr=ymin+ydi*(*RanNum)(r);
        zr=zmin+zdi*(*RanNum)(r);
        #endif
        ...
        for(im=0;im<INOR+1;im++){
            //printf("yr:%le zr:%le\n",yr,zr);
            che(im,newVector,hs,&cof[im+1],cof[im],versor, &yr, &zr, &kp,n_atoms);
            ...
            if(cof[im+1]==cof[im]){
                ...
                break;                
            }else if (im+1>*imm) *imm=im+1;  // if we got a collision (kp=1)
        }
        if(kp==1) crb+=pls; // Projection approximation (= simple monte carlo = count only first collision)
        for(im=0;im<INOR;im++) crof[im] += pls*cof[im+1]*cof[im+1];  // for hard sphere (= count multiple collisions)
    }
    //printf("ok from %d\n",my_rank);
    // END of MONTE CARLO
    
        double g_crof[INOR];
        MPI_Reduce(&crof, &g_crof, INOR, MPI_DOUBLE, MPI_SUM, 0,MPI_COMM_WORLD);
        //for(im=0;im<INOR;im++) g_crof[im] /=((double)INUM*nprocs*0.5);
        
        double g_crb;
        MPI_Reduce(&crb, &g_crb, 1, MPI_DOUBLE, MPI_SUM, 0,MPI_COMM_WORLD);
        // *pacs=g_crb/((double)INUM*nprocs);        
        // *pamob=mol->mobility/(sqrt(mol->temperature)*(*pacs));

        unsigned int g_imm;
        MPI_Reduce(imm, &g_imm, 1, MPI_UNSIGNED, MPI_MAX, 0,MPI_COMM_WORLD);
        // *ehscs=crof[g_imm-1];
        // *imm=g_imm;
...
//=====================================
    ...
    //broadcast data read by rank==0. As we play with struct, we need to send those in MPI format one-by-one
    // Of ourse, I can use MPI_Type_create_struct, but as I need to do this just once and I do not transfer the entire struct
    // This is more efficient

    //inData
    MPI_Bcast(inData.label,MAX_LINE,MPI_CHAR,0,MPI_COMM_WORLD);
    MPI_Bcast(&inData.n_atoms,1,MPI_UNSIGNED,0,MPI_COMM_WORLD);
    MPI_Bcast(&inData.n_coord,1,MPI_UNSIGNED,0,MPI_COMM_WORLD);
    MPI_Bcast(&inData.unit,1,MPI_INT,0,MPI_COMM_WORLD);
    MPI_Bcast(&inData.cUnit,1,MPI_INT,0,MPI_COMM_WORLD);
    MPI_Bcast(&inData.cFact,1,MPI_DOUBLE,0,MPI_COMM_WORLD);
  //  printf("Ok from %d\n",my_rank);
    //lj
    if (my_rank!=0){
        lj.eolj=AllocDoubleVec(inData.n_atoms);
        lj.rolj=AllocDoubleVec(inData.n_atoms);
    }
    MPI_Bcast(lj.eolj,inData.n_atoms,MPI_DOUBLE,0,MPI_COMM_WORLD);
    MPI_Bcast(lj.rolj,inData.n_atoms,MPI_DOUBLE,0,MPI_COMM_WORLD);
    if (my_rank!=0) lj=SetLJ(lj.eolj,lj.rolj,inData.n_atoms);
    
    //hs
    if (my_rank!=0){
        hs.rhs=AllocDoubleVec(inData.n_atoms);
        hs.rhs2=AllocDoubleVec(inData.n_atoms);
    }
    MPI_Bcast(hs.rhs,inData.n_atoms,MPI_DOUBLE,0,MPI_COMM_WORLD);
    MPI_Bcast(hs.rhs2,inData.n_atoms,MPI_DOUBLE,0,MPI_COMM_WORLD);

    //mol
    MPI_Bcast(&mol.TotalMass,1,MPI_DOUBLE,0,MPI_COMM_WORLD);
    MPI_Bcast(&mol.totalAbsoluteCharge,1,MPI_DOUBLE,0,MPI_COMM_WORLD);
    MPI_Bcast(&mol.totalCharge,1,MPI_DOUBLE,0,MPI_COMM_WORLD);
    
    //atoms
    if (my_rank!=0){
        atoms.coord=AllocDoubleMat(inData.n_atoms,3);
        atoms.charge=AllocDoubleVec(inData.n_atoms);
        atoms.mass=AllocDoubleVec(inData.n_atoms);
    }
    MPI_Bcast(&atoms.coord[0][0],inData.n_atoms*3,MPI_DOUBLE,0,MPI_COMM_WORLD);
    MPI_Bcast(atoms.charge,inData.n_atoms,MPI_DOUBLE,0,MPI_COMM_WORLD);
    MPI_Bcast(atoms.mass,inData.n_atoms,MPI_DOUBLE,0,MPI_COMM_WORLD);
...
//    printf("Ok from %d\n",my_rank);
    ...
    if (my_rank==0) fprintf(fpOut," dipole constant =% -.4E\n",mol.dipole);
    ...
    mol.mobility /=(XN/XMV);
    if (my_rank==0) fprintf(fpOut," mobility constant =% -.4E\n",mol.mobility); 
    if (my_rank==0) fprintf(fpOut," temperature =% -.4E\n",mol.temperature);
    ...
    for (iic=0;iic<inData.n_coord;iic++){
        fprintf(fpOut,"\n\n");
        if ((my_rank==0) && (inData.n_coord > 1)) fprintf(fpOut,"\n coordinate set =%5d\n",iic+1);
        if (my_rank==0) fprintf(fpOut,"\n structural asymmetry parameter =%8.4f\n",asympp[iic]);
        #if TEST_CTYPES
        ...
        if (iic<inData.n_coord-1){
            if (my_rank==0) ncoord(&inData,&mol,&atoms,&asympp[iic+1],fpIn,fpOut);            
            MPI_Bcast(&atoms.coord[0][0],inData.n_atoms*3,MPI_DOUBLE,0,MPI_COMM_WORLD);
            MPI_Bcast(atoms.charge,inData.n_atoms,MPI_DOUBLE,0,MPI_COMM_WORLD);
            MPI_Bcast(atoms.mass,inData.n_atoms,MPI_DOUBLE,0,MPI_COMM_WORLD);
        } else if (my_rank==0) fclose(fpIn); //nothing more to read
    ...
    if (my_rank==0) {
        fprintf(fpOut,"\n\n\n SUMMARY\n\n");
        fprintf(fpOut," program version = 0.3 using %d nodes\n",nprocs);
        fprintf(fpOut," input file name = %s\n",inFile);
        fprintf(fpOut," input file label = %s\n",inData.label);
        if(tmmob!=0){
