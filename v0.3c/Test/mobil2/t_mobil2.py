...
#       C wrapper
#======================
class lj_c(ctypes.Structure):
    _fields_=[("eolj",ctypes.POINTER(ctypes.c_double)),("rolj",ctypes.POINTER(ctypes.c_double)),
              ("eox4",ctypes.POINTER(ctypes.c_double)),("ro2lj",ctypes.POINTER(ctypes.c_double)),
              ("ro6lj",ctypes.POINTER(ctypes.c_double)),("ro12lj",ctypes.POINTER(ctypes.c_double)),
              ("dro6",ctypes.POINTER(ctypes.c_double)),("dro12",ctypes.POINTER(ctypes.c_double)),
              ("romax",ctypes.c_double),("n",ctypes.c_size_t)
              ]


class mol_c(ctypes.Structure):
    _fields_=[("TotalMass",ctypes.c_double),("dipole",ctypes.c_double),
              ("TotalCharge",ctypes.c_double),("TotalAbsoluteCharge",ctypes.c_double),
              ("mGas",ctypes.c_double),("mu",ctypes.c_double),
              ("invMu",ctypes.c_double),("mobility",ctypes.c_double),
              ("temperature",ctypes.c_double)
              ]

def mobil2_c(coordList,chargeList,fout,parmList,switch,lj,dipole,ion,mconst,iic,RngGen):
    mob=ctypes.c_double()
    cs=ctypes.c_double()
    sdevpc=ctypes.c_double()
    _doublepp = ndpointer(dtype=np.uintp, ndim=1, flags='C')

    c_rand=ctypes.CFUNCTYPE(ctypes.c_double)
    lj2=lj_c(lj.eolj.ctypes.data_as(ctypes.POINTER(ctypes.c_double)),lj.rolj.ctypes.data_as(ctypes.POINTER(ctypes.c_double)),lj.eox4.ctypes.data_as(ctypes.POINTER(ctypes.c_double)),lj.ro2lj.ctypes.data_as(ctypes.POINTER(ctypes.c_double)),lj.ro6lj.ctypes.data_as(ctypes.POINTER(ctypes.c_double)),lj.ro12lj.ctypes.data_as(ctypes.POINTER(ctypes.c_double)),lj.dro6.ctypes.data_as(ctypes.POINTER(ctypes.c_double)),lj.dro12.ctypes.data_as(ctypes.POINTER(ctypes.c_double)),lj.romax,lj.eolj.shape[0])
    mol2=mol_c(ion.TotalMass,ion.dipole,ion.totalCharge,ion.totalAbsoluteCharge,ion.mGas,ion.mu,ion.invMu,ion.mobility,ion.temperature)

    my_libc.wrap_mobil2.argtypes=[_doublepp,ctypes.POINTER(lj_c),ctypes.POINTER(ctypes.c_double),ctypes.c_size_t,ctypes.POINTER(mol_c),ctypes.c_int,ctypes.POINTER(ctypes.c_double),ctypes.POINTER(ctypes.c_double),ctypes.POINTER(ctypes.c_double),c_rand,ctypes.c_int]
    my_libc.wrap_mobil2.restype=None
    my_libc.wrap_mobil2((coordList.__array_interface__['data'][0]+ np.arange(coordList.shape[0])*coordList.strides[0]).astype(np.uintp),lj2,chargeList.ctypes.data_as(ctypes.POINTER(ctypes.c_double)),coordList.shape[0],mol2,iic,ctypes.byref(mob),ctypes.byref(cs),ctypes.byref(sdevpc),c_rand(rando_c.rand),switch.im2)

    return mob.value,cs.value,sdevpc.value
...
## Set C parameters ##
###########################
rando_c=np.random.RandomState(seed=43)
my_libc=ctypes.CDLL("./libcMobCal.dll")
...
fout_c=open("c_file.dat","w") # file with the result for fortran
...
for iic in range(InputData.icoord):
    ...
    print("Timings for iteration:",iic,"\n=========================")

    start=time.time()
    tmm_c,tmc_c,sdevpc_c=mobil2_c(atoms.Coords,atoms.Charges,fout_c,ConstList,printDebug,lj,ion.dipole,ion,mconst,iic,RngGen)
    end=time.time()
    print("C: ",end-start)
    print(tmm_c,tmc_c,sdevpc_c,file=fout_c)
