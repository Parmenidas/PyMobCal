...
#                  C
#==========================================
class hs_c(ctypes.Structure):
    _fields_=[("rhs",ctypes.POINTER(ctypes.c_double)),("rhs2",ctypes.POINTER(ctypes.c_double))
              ]
...
def mobil4_c(coordList,chargeList,fout,parmList,switch,lj,ion,mconst,RngGen):
    ehscs=ctypes.c_double()
    ehsmob=ctypes.c_double()
    pacs=ctypes.c_double()
    pamob=ctypes.c_double()
    imm=ctypes.c_int()

    hs2=hs_c(lj.rhs.ctypes.data_as(ctypes.POINTER(ctypes.c_double)),lj.rhs2.ctypes.data_as(ctypes.POINTER(ctypes.c_double)))
    ...
    my_libc.wrap_mobil4.argtypes=[_doublepp,ctypes.POINTER(hs_c),ctypes.POINTER(mol_c),c_rand,ctypes.POINTER(ctypes.c_double),ctypes.POINTER(ctypes.c_double),ctypes.POINTER(ctypes.c_double),ctypes.POINTER(ctypes.c_double),ctypes.POINTER(ctypes.c_int),ctypes.c_size_t]
    my_libc.wrap_mobil4.restype=None
    my_libc.wrap_mobil4((coordList.__array_interface__['data'][0]+ np.arange(coordList.shape[0])*coordList.strides[0]).astype(np.uintp),hs2,mol2,c_rand(rando_c.rand),ctypes.byref(ehscs),ctypes.byref(ehsmob),ctypes.byref(pacs),ctypes.byref(pamob),ctypes.byref(imm),coordList.shape[0])

    #(double **coord,struct HardSphere* hs,const struct Molecule *const mol,double (*RanNum)(void),double *ehscs,double *ehsmob,double *pacs,double *pamob,unsigned int * imm,size_t n_atoms,FILE *fp)

    return ehscs.value,ehsmob.value,pacs.value,pamob.value,imm.value
...
ehsc_c=np.zeros(InputData.icoord)
ehsm_c=np.zeros(InputData.icoord)
pac_c=np.zeros(InputData.icoord)
pam_c=np.zeros(InputData.icoord)
...
for iic in range(InputData.icoord):
    ...
    start=time.time()
    ehsc_c[iic],ehsm_c[iic],pac_c[iic],pam_c[iic],imm_c=mobil4_c(atoms.Coords,atoms.Charges,fout_t_py,ConstList,printDebug,hs,ion,mconst,RngGen)
    end=time.time()
    print("C: ",end-start)
    print(ehsc_c[iic],ehsm_c[iic],pac_c[iic],pam_c[iic],imm_c,file=fout_c)
